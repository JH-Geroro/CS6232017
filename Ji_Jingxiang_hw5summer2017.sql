...
SELECT Product.pname FROM Product WHERE prod_id IN (SELECT DISTINCT T1.prod_id FROM Stock T1, Stock T2, Stock T3 WHERE T1.prod_id=T2.prod_id AND T2.prod_id=T3.prod_id AND T1.dep_id<>T2.dep_id AND T2.dep_id<>T3.dep_id AND T1.dep_id<>T3.dep_id);
...
--MINUS is not available in the PostgreSQL. And I have used MINUS in SELECT but there is a syntax errors when I execute it, so I replaced MINUS with INTERSET.
